...
- **Google Search Console + Bing Webmaster Tools bejelentés**: Judit elkezdte, hogy gyorsabban induljon a keresőindexelés (DNS TXT-hitelesítés a cPanel Zone Editorral, sitemap.xml beküldése, "Request indexing" a főoldalra). **Nem tudni, lezárult-e** — ha legközelebb szóba kerül, kérdezz rá, és ha van Google-site-verification kód, azt be kell tenni az `index.html` `<head>`-jébe (meta tag, nem titkos adat, nyugodtan kérhető chatben).
